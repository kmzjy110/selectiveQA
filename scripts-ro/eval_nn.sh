...
for EXPOSE_PREFIX in "${EXPOSE_PREFIXES[@]}"; do
        ...
        python3 run_experiment.py ${MODEL_DIR} qa nn_eval --target_prefix=${INPUT_DATASET_PREFIX} --expose_prefix=${EXPOSE_PREFIX} --output_dir=${OUTPUT_DIR} --bert_model bert-base-uncased --do_lower_case
    done
echo
